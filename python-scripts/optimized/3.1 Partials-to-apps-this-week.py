...
for program_key, data in program_reports.items():
    ...
    print(
        f"SALESFORCE: "
        f"{os.path.basename(salesforce_file)}"
    )
    ...
    # ==================================================
    # FILTER APPLICATIONS TO THE PREVIOUS WEEK
    # ==================================================

    # Salesforce weeks run Sunday through Saturday.
    #
    # Example:
    # Latest report: Monday 08/10/2026
    # Week being reported:
    # Sunday 08/02/2026 - Saturday 08/08/2026

    merged["Application Date Submitted"] = pd.to_datetime(
        safe_column(
            merged,
    ...
    week_start = latest_date - pd.Timedelta(days=8)

    week_end = latest_date - pd.Timedelta(days=2)

    print(
        f"Application date range: "
        f"{week_start.strftime('%m-%d-%Y')} "
        f"to "
        f"{week_end.strftime('%m-%d-%Y')}"
    )

    # Keep ONLY applications submitted
    # during the Salesforce reporting week
    ...
    print(
        f"Partial Entries Converted to Apps This Week: "
        f"{converted_count}"
    )
